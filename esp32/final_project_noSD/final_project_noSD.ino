...
const uint32_t GET_GPS_CYCLE = 1000 * 15;
...
float prev_lats[2];
float prev_lons[2];
float init_lat0 = 42.62512;
float init_lon0 = -71.14477;
float init_lat1 = 42.92512;
float init_lon1 = -72.14477;
...
void setup() {
  ...
  prev_lats[0] = init_lat0;
  prev_lats[1] = init_lat1;
  prev_lons[0] = init_lon0;
  prev_lons[1] = init_lon1;
    ...
    case PUBLIC: {
      ...
      if (millis() - timer >= GET_GPS_CYCLE) {
        ...
        if (gps.location.isValid()) {
          ...
          prev_lats[1] = prev_lats[0];
          prev_lons[1] = prev_lons[0];
          prev_lats[0] = gps.location.lat();
          prev_lons[0] = gps.location.lng();
          ...
          predict_location();
          sprintf(body, "user=%s&lat=%f&lon=%f&confirmed=%b", USER, prev_lats[0], prev_lons[0], confirmed_case); //generate body, posting to User, 1 step
        }
        ...
        char moreinfo[40];
        sprintf(moreinfo, "My location is: %f, %f", prev_lats[0], prev_lons[0]);
        tft.println(moreinfo);
        tft.println();
        timer = millis();
      }
      break;
...
void predict_location() {
  Serial.println("predicting location...");
  float latdiff = prev_lats[1] - prev_lats[0];
  float londiff = prev_lons[1] - prev_lons[0];
  Serial.println(latdiff);
  Serial.println(londiff);
  float dist = pow(pow(latdiff, 2) + pow(londiff, 2),0.5);
  float angle = tan(londiff/latdiff);
  float latadd = dist*cos(angle);
  float lonadd = dist*sin(angle);
  prev_lats[1] = prev_lats[0];
  prev_lons[1] = prev_lons[0];
  prev_lats[0] = prev_lats[0] + latadd;
  prev_lons[0] = prev_lons[0] + lonadd;
  Serial.println("New coords: %f, %f");
  Serial.println(prev_lats[0]);
  Serial.println(prev_lons[0]);
}
